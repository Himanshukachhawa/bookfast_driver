...
class EditPhoneNumber extends Component<Props> {
 ...
 update_data = async () => {
    ...
    await axios({
      ...
      data:{ driver_id : global.id, phone_number : this.state.phone, phone_with_code: '+'+this.phone.getCountryCode()+this.state.phone } 
    })
    .then(async response => {
      ...
      <View style={{backgroundColor:colors.theme_fg_three,height:"100%",width:"100%" }}>
        <View style={{backgroundColor:colors.theme_fg_three}}>
          <View style={{ padding:20, height:height_40 }}>
            ...
            <PhoneInput style={{ borderColor:colors.theme_fg_two }} flagStyle={styles.flag_style} ref={(ref) => { this.phone = ref; }} value={this.state.phone} initialCountry="in" offset={10} textStyle={styles.country_text} textProps={{ placeholder: 'Phone number', placeholderTextColor : colors.theme_fg_four }} autoFormat={true} />
