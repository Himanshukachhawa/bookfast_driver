...
import { font_title, font_description, LATITUDE_DELTA, LONGITUDE_DELTA, img_url, status_change, get_stops, api_url, GOOGLE_KEY, alert_close_timing, get_drop_addresses, trip_cancel, cancel_ride} from '../config/Constants';
...
class Trip extends Component<Props> {
  ...
  booking_sync = () =>{
    let onValueChange = database().ref('/trips/'+this.state.trip_id).on('value', snapshot => {
      if(this.state._isMounted) {
      this.setState({ data : snapshot.val() });
      if(snapshot.val().status == 5){
        //this.home();
        this.props.navigation.navigate('Rating',{ data : snapshot.val() });
      }
      if(snapshot.val().status == 6){
    ...
    fonValueChange = database().ref('/chat/'+this.state.trip_id)
      .limitToLast(1)
              ...
              <View style={styles.map_container}>
                ...
                >
                  ...
                  {global.polyline_status == 1 &&
                    <PolylineDirection
                      origin={ this.state.data.status <= 2 ? this.createMarker(parseFloat(this.state.data.pickup_lat),parseFloat(this.state.data.pickup_lng)) : this.createMarker(parseFloat(this.state.data.drop_lat),parseFloat(this.state.data.drop_lng)) }
